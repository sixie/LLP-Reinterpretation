...
def two_d_arraymaker_row3(dataframe, kind=''): # extrapolates cross section values according to a quadratic relationship with lambda
    tdframe = two_d_arraymaker_row2(dataframe)
    rowlist = []
    if kind == 'Q' or kind == '':
        for lam in np.linspace(0.002, 0.25, 100):
            rowlist.append(lam)
            for i in range(len(tdframe[0]) - 1):
                rowlist.append((lam / tdframe[1][0]) ** 2 * tdframe[1][i+1])
            tdframe.append(rowlist)
            rowlist = []
    elif kind == 'G':
        for lam in np.linspace(0.002, 0.25, 100):
...
def two_d_arraymaker(fignum, plotnum, date, kind=''): # produces plotting array based on figure number, plot number, date, and kind ('Q' or 'G')
    ...
    return pd.DataFrame(two_d_arraymaker_row3(df, kind))